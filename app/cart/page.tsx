...
export default function CartPage() {
  const [items, setItems] = useState([
    { id: "wheat", name: "Premium Wheat Seeds", price: 45, quantity: 50, image: "https://images.unsplash.com/photo-1595185495541-11a3d9bc3df0?auto=format&fit=crop&q=80&w=200" }
  ]);

  const updateQuantity = (id: string, delta: number) => {
    setItems(items.map(item => {
      if (item.id === id) {
        const newQ = Math.max(5, item.quantity + delta);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <div key={item.id} className="bg-white rounded-[24px] p-4 sm:p-6 border border-[#EEE6D8] shadow-soft flex flex-col sm:flex-row gap-6 items-center sm:items-start">
                  ...
                  <div className="flex-grow flex flex-col justify-between w-full">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <button onClick={() => removeItem(item.id)} aria-label={`Remove ${item.name}`} className="text-[#A63D2F] hover:text-[#8B3125] p-2">
                    ...
                    <div className="flex justify-between items-center mt-auto">
                      <div className="flex items-center border border-[#DDD3C3] rounded-[10px] overflow-hidden">
                        <button onClick={() => updateQuantity(item.id, -1)} aria-label={`Decrease quantity of ${item.name}`} className="p-2 hover:bg-[#F3EEDF] transition">
                          <Minus size={16} aria-hidden="true" />
                        </button>
                        <div className="w-12 text-center font-bold text-sm" aria-label={`Quantity: ${item.quantity} kg`}>{item.quantity} kg</div>
                        <button onClick={() => updateQuantity(item.id, 1)} aria-label={`Increase quantity of ${item.name}`} className="p-2 hover:bg-[#F3EEDF] transition">
            ...
            <div className="bg-white rounded-[24px] p-6 border border-[#EEE6D8] shadow-soft sticky top-24">
              ...
              <Link 
                ...
                className={`w-full py-4 rounded-[14px] transition shadow-btn font-medium text-lg flex justify-center items-center ${
                  items.length > 0 ? "bg-[#A63D2F] text-white hover:bg-[#8B3125]" : "bg-gray-300 text-gray-500 cursor-not-allowed"
                }`}
              >
                Proceed to Checkout
              </Link>
